---
title: Craft Freeform 5.x - Form Builder
...
<div class="menu-grid">
    ...
    <a href="#email-notifications" class="menu-box">
        <img src="../../../../images/icons/email-group.png" alt="Email Notifications">
        <div class="menu-grid-text">
            <h3>Email Notifications</h3>
            <p>Configure all email notification types for the form.</p>
        </div>
    </a>
    <a href="#conditional-rules" class="menu-box">
        ...
        <div class="menu-grid-text">
            ...
            <p>Set which fields and pages should be shown/hidden.</p>
        </div>
    </a>
    <a href="#integrations" class="menu-box">
        <img src="../../../../images/icons/crm.png" alt="Integrations">
        <div class="menu-grid-text">
            <h3>Integrations</h3>
            <p>Configure various integrations for your form.</p>
        </div>
    </a>
    ...
    <a href="#form-types" class="menu-box">
...
This is where you set up [fields](../fields) and [pages](#multi-page-forms), as well as design and configure the overall layout of the form.
...
- **Preview** - this is where you will actively see an interactive live preview of what your form will look like (roughly - ultimately the form styling and appearance is up to your [formatting template](./formatting-templates/)).
...
Email notifications can be sent to one or more pre-defined admin email addresses that are selected by the user filling out the form using any [Dropdown](../fields/#dropdown), [Checkboxes](../fields/#checkboxes), [Radios](../fields/#radios), [Multi-Select](../fields/#multiple-select) field types. For example, you might have a **Dropdown** field that contains different departments for the user to address the email to.
...
::: warning
If you currently don't have any **Email** fields, please add one to your form layout. If you are using a regular [Text](./fields/#text) field to hold the email address, please switch it to an [Email](./fields/#email) field type first.
:::

You can also enable "tell-a-friend" type forms by including additional [Email](./fields/#email) fields in your form. This would then allow the submitter of the form to specify a friend's email address and have them receive an email notification as well.
...
This is where you can configure various types of integrations for your form. It will only show integrations that have been configured/setup in the [Freeform Settings](../configuration/settings/) area.
...
Please see the [Spam Protection](./spam-protection/) documentation guide for more information about this feature.
...
Please see the [Stripe Payments](../integrations/payments/) documentation guide for further instruction.
...
Please see the [Elements](../integrations/elements/) documentation for more information about this feature.

### Google Tag Manager <div class="badge-group"><Badge type="lite" text="Lite" /><Badge type="pro" text="Pro" /></div>
...
Please see the [POST Forwarding](../integrations/post-forwarding/) documentation for more information about this feature.
...
- **Formatting Template** - if using the [render() method](../templates/queries/form/#render), this essentially allows you to "attach" a formatting template to a form so that you don't need to include formatting inside the template(s) you place the form inside.
...
- **Store Submitted Data for this Form** - allows you to specify whether the submission data for this form should be stored in the database.
    - Typical use cases for not storing submission data on your site would be if you just wanted the [email notification](./email-notifications/) and/or wanted the data submitted over to an [Email Marketing](../integrations/) or [CRM](../integrations/) integration.
    - It's important to note that this likely isn't a complete solution for [GDPR Compliance](../configuration/gdpr-compliance/) if that is why you are using this feature. Please read our official [Guide to GDPR Compliance with Freeform](../configuration/gdpr-compliance/) for more information.
- **Default Status** - the default status to be used when users submit the form, unless overwritten at template level.
    - See [Statuses](../configuration/settings/#statuses) documentation for managing statuses.
...
- **Allow Users to Opt-in** - if enabled, it allows the submitter to choose whether or not they want their submission data stored on your site.
    ...
    - If the user does NOT opt-in, Freeform will still send off data to [integrations](../integrations/) and [email notifications](./email-notifications/).
    - It's important to note that this likely isn't a complete solution for [GDPR Compliance](../configuration/gdpr-compliance/) if that is why you are using this feature. Please read our official [Guide to GDPR Compliance with Freeform](../configuration/gdpr-compliance/) for more information.

### Processing

- **Use AJAX** - enables Freeform's [built-in automated AJAX submit support](../templates/ajax-forms/). Once enabled, Freeform will handle the rest (as long as you're using standard template formatting - very custom formatting templates may not work properly).
...
- **Success Behavior** - set how you'd like the success return of this form to be handled. Some of these may also conflict with template-level approaches to controlling the form behavior, most typically [JS Overrides](../templates/ajax-forms/).
    - Options are:
        ...
        - **Replace form with a Success message** - upon successful submit of the form, Freeform will stay on the same page but replace the form contents with the contents of the _Success Template_ that you specify. Please note that this will not work when manually rendering form formatting directly inside a regular template (not using a [Formatting Template](./formatting-templates/)).
            - **Success Template** - specify which [Success Template](../configuration/settings/#template-manager) you'd like to be used for this form.
...
Please see the [Submission Limits](./submission-limits/) documentation for more information about available options, including setting limits for the entire form.
...
## Multi-Page Forms <div class="badge-group"><Badge type="lite" text="Lite" /><Badge type="pro" text="Pro" /></div>
...
<video autoplay loop muted>
    <source src="../videos/builder-pages.mp4" type="video/mp4">
    This browser does not display the video tag.
</video>
...
- Multi-page forms work with the [built-in AJAX](../templates/ajax-forms/) feature.
	- **Editing** existing submissions via the front end in multi-page AJAX forms will currently not work correctly.
- To allow users to stop and return at a later time to complete the form, it will require use of the [Save & Continue Later](../forms/save-continue-later/) feature. Otherwise, the process has to be a continuous one, but the user has 3 hours before the form will timeout the submission.
- If an earlier page contains file upload field(s), files will actually be uploaded before the form is officially submitted.
	...
	- If you require a longer period of time, we recommend using the [File Upload Drag & Drop](./fields/#file-upload-drag--drop) field and the [Save & Continue Later](../forms/save-continue-later/) feature.

### Manual Submit Buttons in Multi-page Forms
When manually building submit buttons in multi-page forms, please refer to the [Submit buttons](./fields/#submit-buttons) guide for available options.
...
<video autoplay loop muted>
    <source src="../videos/form-dashboard.mp4" type="video/mp4">
    This browser does not display the video tag.
    <img src="../images/cp/form-dashboard.png" title="Your browser does not support the <video> tag">
</video>

### Errors & Update Notices

The top of the **Forms** page will conveniently let you know if something is wrong by displaying a banner indicating the number of errors that Freeform logged, if available, as well as [Update Notices](../configuration/notices-alerts/) to keep you informed about issues that may specifically affect your site.
...
- Stats:
    ...
    - A chart (matching the color set inside the form builder) at the bottom of the card indicating submission counts over the past year.
        - Chart colors are controlled by the **Form Color** setting inside the form builder for each form (_Settings_ → _General_).
