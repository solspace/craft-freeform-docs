...
:::info
If you're using Freeform 5.3+, this issue is very unlikely to occur. The _Freeform Script Insert Type_ setting was improved and no longer offers the **Static URLs** option, which was likely to cause this issue in some environments. The below instructions only apply to versions before 5.3.
:::
...
- **Form/template/website is cached** - if using cache, please see the [Caching Forms](../guides/templating/caching-forms/) documentation guide to see how you will need to manually refresh the Craft CSRF token.
