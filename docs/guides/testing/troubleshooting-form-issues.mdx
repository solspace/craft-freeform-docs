...
3. The [Freeform Script Insert Location](../../configuration/settings.mdx#script-insert-location) setting is set to `Manual`. Most sites should use `Page Footer`, but in some cases it may be necessary to load these manually. If you must use `Manual`, please make sure you're loading Freeform's scripts in your forms with `freeform.loadFreeformPlugin()`. Please see the guide in the [JS Plugin documentation](../../developer/js-plugin.mdx#loading-freeform-js-manually) for more info.
