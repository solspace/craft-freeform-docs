...
- **Payments**: When using Freeform's [Stripe Payments](../integrations/stripe-payments/) feature, Freeform will automatically force AJAX mode on the form in order to work with Stripe's validation.
...
::: tip
Be sure to check out the [Freeform Javascript plugin](../developer/js-plugin) documentation, to learn how to further extend Freeform's functionality.
:::
...
2. Inside your regular template:
   ...
   - Add the `formAttributes: { "data-skip-html-reload": true },` parameter to your [Form query](../templates/queries/form).

## Advanced

To attach your own functionality when the AJAX form successfully submits or fails, you have to listen to the respective events dispatched by the [Freeform JS plugin](../developer/js-plugin). For instance, redirecting after an AJAX submit is finished:
