...
```yaml {2,9,15,18,23,29,37,41,49,53,63,80,89} showLineNumbers
freeform:
  # GENERAL INFO
  edition: pro
  enabled: true
...
```_ {2,9,15,18,23,29,37,41,49,53,63,80,89}
...
← NEW IN 5.0+  Set defaults and locks for form builder settings.
  ...
  ← FORM BUILDER SETTING DEFAULTS
    ...
    ← Duplicate Check
      ← Options are 'no_limit', 'no_limit_logged_in_users_only', 'limit_once_per_logged_in_user_only', 'limit_once_per_email', 'limit_once_per_user_or_cookie', and 'limit_once_per_user_or_ip_or_cookie'.
```
