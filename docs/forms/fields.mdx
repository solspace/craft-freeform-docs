...
:::tip
[Need to build your own custom field type? Check out the guide.](../developer/field-types/)
:::
...
- All field types conveniently include the ability to set attributes for their **labels**, **inputs**, **errors** and **instructions** directly inside the form builder property editor.
  ...
  - You can use anything inside the [Form](../templates/objects/form) and [Field](../templates/objects/fleld) objects as well, e.g.:
...
- [Need to display basic HTML (`<b>`, `<i>`, `<a>`, etc) in your option labels to add emphasis or display a link?](../user-guides/guides/field-styling/parsing-html-in-field-options/)
- [Want to display single checkbox fields as a light switch toggle?](../user-guides/guides/field-styling/toggle-switch-checkbox/)
...
- [Need to display basic HTML (`<b>`, `<i>`, `<a>`, etc) in your option labels to add emphasis or display a link?](../user-guides/guides/field-styling/parsing-html-in-field-options/)
- [Need to populate field options at template level? Check out the guide for workaround options.](../user-guides/guides/dynamic-data/populate-field-options-at-template-level)
...
- [Need to populate field options at template level? Check out the guide for workaround options.](../user-guides/guides/dynamic-data/populate-field-options-at-template-level)
- [Looking to include an additional "Other" choice that displays an input field for users to fill in their own value?](../user-guides/guides/field-styling/other-option-for-fields)
...
:::tip
[Need to prevent users from using certain types of email addresses (e.g. common free ones such as Gmail, Yahoo, etc)?](../user-guides/guides/form-validation/preventing-email-addresses)
:::
...
:::tip
[Want to display a Number field as a range slider?](guides/range-slider-number/)
:::
...
A special field type that allows you to securely collect a password from a submitter **but NOT store the value in the Freeform database** (typically used for [User Registration forms](../user-guides/guides/speciality-forms/user-registration-forms) with the [Elements integration](../integrations/elements/)). Pair this with the [Confirm](#confirm) special field type if you wish to have the user enter their password twice. The `field.type` property value is `password` (for conditionals).
...
- [Need to display basic HTML (`<b>`, `<i>`, `<a>`, etc) in your option labels to add emphasis or display a link?](../user-guides/guides/dynamic-data/populate-field-options-at-template-level)
- [Need to populate field options at template level? Check out the guide for workaround options.](../user-guides/guides/dynamic-data/populate-field-options-at-template-level)
- [Looking to include an additional "Other" choice that displays an input field for users to fill in their own value?](../user-guides/guides/field-styling/other-option-for-fields)
...
:::tip
[Want to allow front end users to use a rich text editor such as **TinyMCE** on a textarea field?](../user-guides/guides/field-styling/tinymce-editor-textarea)
:::
...
When the [Stripe Payments](../integrations/stripe-payments) integration is configured, a Stripe Payments special field type will become available. This field enables your forms to collect (but not store) credit card or other payment information and pass it off to Stripe.
...
:::tip
[Want to display a Text input as a Color Picker?](guides/color-picker-text/)
:::
...
- [Need to display linebreaks for Textarea fields in email notifications or on the front end with a Submissions query?](../user-guides/guides/templating/line-breaks-in-textarea-fields)
- [Want to allow users to use a rich text editor such as **TinyMCE** on the textarea field?](../user-guides/guides/field-styling/tinymce-editor-textarea)
...
:::tip
[Need more control over populating field options dynamically at template level, such as from a Craft Entry or some other Twig code in your template? Check out how to populate field options at template level.](../user-guides/guides/dynamic-data/populate-field-options-at-template-level)
:::
