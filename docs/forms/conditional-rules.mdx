...
import { VerticalStepWithLabel } from '@site/src/components/docs';
...
:::tip User Guides
[Quick troubleshooting the most commonly reported issues with your form's appearance, behavior, or submission of the form on the front end.](https://docs.solspace.com/craft/freeform/v5/guides/troubleshooting-form-issues/)
:::
...
- To set rules for a [Group](/forms/conditional-rules#group) field, click on the group field name in the left column.
...
- Criteria values/options will automatically display as the following: - Text input (where you type out the criteria) for basic text/number field types. - Select dropdown (where you select a predefined option as criteria) for multi-option field types such as Checkboxes, Radios, Dropdowns, etc. - Checkbox (where you check the checkbox) for single Checkbox field types.
  :::
...
<VerticalStepWithLabel
  steps={[
    'Click the <strong>Rules</strong> tab in the form builder. Then choose a field, group or page from the field layout map on the left.',
    'The field, group or page you selected will then load in the right column. Click the <strong>Add rules</strong> button and configure which field(s) will affect whether the selected field will be <strong>shown or hidden</strong>, based on the criteria set for each.',
    'If you wish to add more than one condition for a field, click the <strong>Add a Condition</strong> button. If you have more than one condition for a field, you may also set whether <strong>all</strong> or <strong>any</strong> of the conditions need to be met.',
  ]}
/>
...
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| `myFieldHandle.renderContainerOpeningTag()` | Automatically generates the field wrapper `div`. Field attributes can still be applied to it. Maximum compatibility with attributes specified in the form builder and template overrides, etc. | A bit uglier to look at.                                                                                                                                                                                                                                                                                                                                                        | [Example](#rendercontaineropeningtag) |
