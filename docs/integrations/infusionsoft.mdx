...
- Select _Infusionsoft (v1)_ from the **Service Provider** select dropdown.
...
- Infusionsoft runs its API through Mashery, so you will need to sign up or log into a Mashery account inside the Infusionsoft Developers site.
- [Visit the Infusionsoft Developers site to sign up for or log into a Mashery account](https://keys.developer.infusionsoft.com/apps/myapps)
...
- Copy the `client_id` value from Infusionsoft and paste it into the **Client ID** field in Freeform.
- Copy the `client_secret` value from Infusionsoft and paste it into the **Client Secret** field in Freeform.
