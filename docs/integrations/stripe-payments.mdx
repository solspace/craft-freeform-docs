...
import KeyFeaturesIcon from './assets/icons/list-checks.png';
import RequirementsIcon from './assets/icons/clipboard.png';
import BestPracticesIcon from './assets/icons/star.png';
import CustomizationIcon from './assets/icons/pallete.png';
import SettingUpIcon from './assets/icons/box.png';
import TemplatingIcon from '../assets/icons/code.png';
import EmailNotificationsIcon from '../assets/icons/open-letter.png';
import TestingIcon from './assets/icons/review.png';
...
Freeform Payments uses the [Payment Element](https://stripe.com/docs/payments/payment-element) approach from [Stripe](https://stripe.com). The Payment Element is a UI component that accepts 40+ payment methods, validates input, and handles errors. This means that in addition to accepting **credit cards**, it will allow you to enable support for **Stripe Link**, **Apple Pay**, **Google Pay**, **PayPal** (within Europe), **bank payments**, **deferred payments** and many other options. Within minutes you can have anything from a form accepting donations to a [membership registration form](../user-guides/guides/speciality-forms/user-registration-forms) that has users pay for a subscription at any interval. The possibilities are endless!
