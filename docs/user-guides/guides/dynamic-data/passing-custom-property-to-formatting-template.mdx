...
<VerticalStepWrapper>
<StepMarkdown stepTitle="Step 1">
In your custom [formatting template](../../../templates/formatting/), add in a custom property. For the purpose of this example, we'll set up a conditional around the form title. We'll call that property `showFormTitle`. It can be accessed through `form.properties`.
...
:::info
You can add as many custom properties as you like, but be sure not to use any reserved words such as `class`, `id`, etc (anything that is already a parameter/property name in the [Form query](../../../templates/queries/formx)).
:::
