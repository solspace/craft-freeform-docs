...
Freeform allows you to include config overrides of [Freeform settings](../configuration/settings) in a configuration file, if you wish to do so instead of setting in the database. Just create a new file named `freeform.php` and place it into your Craft `/config/` directory.
