...
The Freeform [Date & Time](../forms/fields#date--time) field comes packed with a built-in, optional [flatpickr](https://flatpickr.js.org/) instance. The _flatpickr_ instance can be customized with via the following events available through the [Freeform JS Plugin](./js-plugin):

:::info
**You're viewing the v3.9 version of these docs**. To switch to the v3.10 docs, [click here to view the revised v3.10 documentation](./datepicker) that shows usage of the Datepicker with the improved Freeform Javascript plugin.
:::
