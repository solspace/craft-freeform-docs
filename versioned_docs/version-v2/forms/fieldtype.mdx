...
- `render()` <a href="#param-render" id="param-render" class="docs-anchor">#</a>
  - Outputs the full form, rendering it with the [Formatting Template](../templates/formatting/) specified in the form builder for the form.
...
The Freeform Submissions field type by itself is not the most usable feature, but when paired with the [Relations](../submissions/relations) feature it allows Freeform to do extraordinary things like become a tool that let's your users comment on entries, submit ratings and reviews for Commerce (and regular) products and more! The [Relations](../submissions/relations) feature essentially allows you to feed another Craft Element ID to the form that is attached to the submission(s), and the Freeform submission then becomes attached to the other element the same way an Asset does, etc.
...
Please see the [Relating Submissions to Elements](../submissions/relations) documentation for a complete guide on setting this up.
