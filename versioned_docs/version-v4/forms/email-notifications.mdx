...
import EmailNotificationsSendAdditionalPhoto from './assets/img/email-notifications-send-additional.png';
...
:::tip
[Email notifications may not work as expected when using **Sendmail** on your local dev environment. Check out our _Mailtrap_ setup guide.](../guides/testing/email-testing-mailtrap)
:::

:::tip
[Need to use the submitter of the form as Reply-to email address in admin email notifications?](../guides/best-practices/admin-notifications-replyto-submitter)
:::
...
<i>Video: Overview of Email Notifications</i>
:::
...
1. In your Twig template, add the following parameter to your [Form query](../templates/queries/form/):
...
Email notifications can be sent to the user submitting the form using the [Email](./fields#email) field type. This is often used to send an email confirmation for the user. To setup:
...
Email notifications can be sent to email addresses of the submitters choosing using the [Email](./fields#email) field type. This would be commonly used for "tell-a-friend" type forms, or forms to send out any other type of invites. The user submitting the form would enter an email address in the form and Freeform can send an email notification to them. This essentially works the same way as [User/Submitter Notifications](#user-submitter-notifications). Just be careful as this could be abused by spammers. To set up:
...
- **Email Body (HTML)** - the HTML body of the email notification to be sent.
  ...
  - May also use `allFields` or `allFieldsAndBlocks` variables to automate parsing of fields.
    ...
    - Excludes _Signature_ fields. To render manually, use:
      ...
      ```
  - You may iterate over submission values from the [Submission object](../templates/objects/submission/) (as long as you're [storing submitted data](./builder/#settings))
  - If using the [Elements integration](./fieldtype), you may use the `element` object to display data from the newly created Craft Element such as ID, title, username, etc.
...
You may also have your notification email template code iterate over submission values from the [Submission object](../templates/objects/submission/) (as long as you're [storing submitted data](./builder/#settings)) like this:
...
<Photo
  img={EmailNotificationsSendAdditionalPhoto}
  alt="Send Additional Email Notifications"
/>
