...
- All fieldtypes conveniently include the ability to set attributes for their **labels**, **inputs**, **errors** and **instructions** directly inside the form builder property editor.
  ...
  - You can use anything inside the [Form](../templates/objects/form) and [Field](../templates/objects/field) objects as well, e.g.:
...
<Photo img={CheckboxPhoto} alt="Checkbox" />

:::tip
[Need to display basic HTML (`<b>`, `<i>`, `<a>`, etc) in your option labels to add emphasis or display a link?](../guides/field-styling/parsing-html-in-field-options)
:::

:::tip
[Want to display single checkbox fields as a light switch toggle?](../guides/field-styling/toggle-switch-checkbox)
:::
...
<Photo img={CheckboxGroupPhoto} alt="Checkboxes field type" />
...
:::tip
[Need to populate field options at template level? Check out the guide for workaround options.](../guides/dynamic-data/populate-field-options-at-template-level)
:::
...
<Photo img={DateTimePhoto} alt="Date & Time field type" />
...
<Photo img={DynamicRecipientsPhoto} alt="Dynamic Recipients" />
...
<Photo img={EmailPhoto} alt="Email field type" />
...
<Photo img={FileUploadPhoto} alt="File upload field type" />
...
- If you wish to allow more file extensions, you will need to add the [extraAllowedFileExtensions](https://craftcms.com/docs/5.x/reference/config/general.html#extraallowedfileextensions) (and possibly [extraFileKinds](https://craftcms.com/docs/5.x/reference/config/general.html#extrafilekinds)) config items to your Craft config file (`config/general.php`).
  ...
  - E.g. To add an assortment of extra file extensions, you add the following to your `config/general.php` file:
    ...
    **In Freeform 4.1.10+:** <Badge type="feature" text="Improved 4.1.10+" /> <br />
...
<Photo img={FileDragDropPhoto} alt="File Upload Drag & Drop field type" />
...
<Photo img={MultipleSelectPhoto} alt="Multiple Select field type" />
...
<Photo img={NumberPhoto} alt="Number field type" />

:::tip
[Want to display a Number field as a range slider?](../guides/field-styling/range-slider-number)
:::
...
<Photo img={OpinionScalePhoto} alt="Opinion scale field type" />
...
<Photo img={PhonePhoto} alt="Phone field type" />
...
<Photo img={RadiosPhoto} alt="Radios field type" />
...
:::tip
[Need to display basic HTML (`<b>`, `<i>`, `<a>`, etc) in your option labels to add emphasis or display a link?](../guides/dynamic-data/populate-field-options-at-template-level)
:::
:::tip
[Need to populate field options at template level? Check out the guide for workaround options.](../guides/dynamic-data/populate-field-options-at-template-level)
:::
:::tip
[Looking to include an additional "Other" choice that displays an input field for users to fill in their own value?](../guides/field-styling/other-option-for-fields)
:::
...
<Photo img={RatingPhoto} alt="Rating field type" />
...
<Photo img={RegexPhoto} alt="Regex field type" />
...
<Photo img={DropdownPhoto} alt="Dropdown field type" />
...
<Photo img={SignaturePhoto} alt="Signature field type" />
...
<Photo img={TablePhoto} alt="Table field type" />
...
Here's how to render a Table field's data inside the [Submission object](../templates/objects/submission):
...
<Photo img={TextFieldPhoto} alt="Text field type" />

:::tip
[Want to display a Text input as a Color Picker?](../guides/field-styling/color-picker-text)
:::
...
<Photo img={TextareaPhoto} alt="Textare field type" />
...
:::tip
[Want to allow users to use a rich text editor such as **TinyMCE** on the textarea field?](../guides/field-styling/tinymce-editor-textarea)
:::
...
<Photo img={WebsitePhoto} alt="Website field type" />
...
<Photo img={ConfirmPhoto} alt="Confirm field type" />
...
When a [Stripe Payments](../integrations/stripe-payments) integration is configured, a credit card special field type will become available. This is required for _Payments_-enabled forms to collect (but not store) credit card information. The `field.type` property value is `cc_details` (for conditionals).

<Photo img={CreditCardPhoto} alt="Stripe Payment field Layout Options" />
...
<Photo img={HTMLPhoto} alt="HTML field type" />
...
<Photo img={EmailMarketingPhoto} alt="Mailing list field type" />

:::tip
[Want to display this checkbox as a light switch toggle?](../guides/field-styling/toggle-switch-checkbox)
:::
...
A special field type that allows you to securely collect a password from a submitter **but NOT store the value in the Freeform database** (typically used for [User Registration forms](../guides/speciality-forms/user-registration-forms) with the [Elements integration](../integrations/elements/)). Pair this with the [Confirm](#confirm) special field type if you wish to have the user enter their password twice. The `field.type` property value is `password` (for conditionals).

<Photo img={PasswordPhoto} alt="Password field type" />
...
A special field type that displays and handles the [reCAPTCHA](https://www.google.com/recaptcha) or [hCaptcha](https://www.hcaptcha.com) interaction with the user when the [Checkbox](./spam-protection) option is enabled and configured for the website. Please note that reCAPTCHA v2 Invisible, reCAPTCHA v3 and hCaptcha Invisble do not require any addition of a special field to the form layout. Freeform will automatically apply that validation and take care of it. The `field.type` property value is `recaptcha` (for conditionals).

<Photo img={CaptchaPhoto} alt="reCAPTCHA v2 Checkbox field type" />

#### Special Notes

- See [Captchas documentation](./spam-protection#captchas) for more information.
...
:::tip
[Want to allow front end users to use a rich text editor such as **TinyMCE** on a textarea field?](../guides/field-styling/tinymce-editor-textarea)
:::

### Save & Continue Later<Badge type="pro" text="Pro" />

A special field type that lets you place "save" button(s) inside your form layout in the form builder, allowing users to save their form progress and continue/complete the form later. For more information about this feature, please see the [Save & Continue Later documentation](./save-continue-later). The `field.type` property value is `save` (for conditionals).

<Photo img={SaveContinueLaterPhoto} alt="Save & Continue later field type" />
...
- You may optionally specify that an email notification be sent to the user if they click the **Save** button.
  ...
  - Requires targeting of a [Email](./fields#email) field that will contain the user's email address in order to send the email notification.
...
<Photo img={SubmitPhoto} alt="Submit Buttons field type" />
...
- For the [Save & Continue Later](./save-continue-later) buttons, the value has to be `save`
...
:::tip
[Need more control over populating field options dynamically at template level, such as from a Craft Entry or some other Twig code in your template? Check out how to populate field options at template level.](../guides/dynamic-data/populate-field-options-at-template-level)
:::
