...
The token generated for the submission. A common use case for tokens would be when you want to have a more secure URL for accessing and displaying submission data, or allowing users to [delete their own submission](../queries/deleting-submissions-by-token) and not requiring any login, etc.

Also required if you wish to allow [editing of submissions on front end](../../submissions/overview/#editing-submissions/).

### `fieldMetadata`

A list containing all fields who store values (doesn't include HTML fields, submit fields, email-marketing fields). Each of the objects is a [Field](./field) object, and contains the submitted value. You can access all of the field properties for each field specific to the current submissions related Form.
