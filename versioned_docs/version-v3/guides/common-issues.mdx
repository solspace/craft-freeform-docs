...
3. [Build a custom module](../guides/developer/building-a-custom-module) that sends an email notification to a recipient based upon the selection of a Select/Checkbox Group/Radio fields' option using the `EVENT_AFTER_SUBMIT` event. We've created [a guide](../guides/developer/building-a-custom-module) that will get you 99% of the way there.
