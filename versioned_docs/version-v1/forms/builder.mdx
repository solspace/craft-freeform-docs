...
import { BadgeGroup, Photo } from '@site/src/components/utils';
...
* **Store Submitted Data** <a href="#submission-store-data" id="submission-store-data" class="docs-anchor">#</a>
  - This setting allows you to specify whether the submission data for this form should be stored in the database or not.
    - Typical use-cases for not storing submission data on your site would be if you just wanted the [email notification](./email-notifications.mdx) and/or wanted the data submitted over to a [Mailing List](../integrations#email-marketing) or [CRM](../integrations#crm) API.
...
* **Return URL** <a href="#form-return-url" id="form-return-url" class="docs-anchor">#</a>
  ...
  - May contain `{{ form.handle }}` and/or `{{ submission.id }}` to parse the newly created unique submission ID in the URL. This would allow you to use the [freeform.submissions](../templates/queries/submissions/) template function to display some or all of the users' submission on the success page.
...
Special Fields include _HTML_ and _Submit_ button(s):
...
If you have installed and configured any [CRM API integrations](../configuration/settings#crm) (Customer Relationship Management), you will see a **CRM** button at the top of the **Property Editor** area (right column). To configure this, click that button, and then select an integration name from the options. You may then map out your Freeform fields to your CRM's fields.
